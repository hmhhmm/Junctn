    ...
    <div className="mx-auto max-w-[1200px] px-6 py-6">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <MetricCard
          ...
          hint="In your book this quarter"
          icon={Users}
        />
        <MetricCard
          label="Open referrals"
          value={brief.stats.openReferrals}
          hint="Suggested, introduced or in progress"
          icon={Network}
          tone="accent"
        />
        <MetricCard
          label="CPD this quarter"
          value={`${brief.cpd.earned}/${brief.cpd.required}`}
          icon={GraduationCap}
          tone={brief.cpd.remaining > 0 ? "warn" : "neutral"}
          progress={brief.cpd.pct}
          progressTone={brief.cpd.remaining > 0 ? "warn" : "ok"}
          hint={`${brief.cpd.daysToDeadline} days to deadline`}
        />
        <MetricCard
          label="Ecosystem gap"
          value={topGap ? `${topGap.specialty}` : "None"}
          hint={
            topGap
              ? `No partner in ${topGap.region} · ${topGap.demand} clients affected`
              : "Full coverage"
          }
          icon={AlertTriangle}
          tone="alert"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {s.payload.detail && (
                        <p className="mt-0.5 text-[12px] text-ink-faint">{s.payload.detail}</p>
                      )}
                    </div>
        ...
        <div className="flex flex-col gap-5">
          <Card id="partners">
            ...
            <CardContent className="flex flex-col gap-3 pt-0">
              {matches.map((s) => (
                <PartnerMatchCard key={s.id} suggestion={s} />
              ))}
            </CardContent>
          </Card>
